...
export class SajuCalculator {
  ...
  // 지지 (12개)
  static EARTHLY_BRANCHES = ['자', '축', '인', '묘', '진', '사', '오', '미', '신', '유', '술', '해'];
  ...
  // 월간지 계산
  static getMonthPillar(yearHeavenly: string, month: number): { heaven: string; earth: string } {
    // 월지 (정월부터 시작)
  ...
  // 시간지 계산
  static getTimePillar(dayHeavenly: string, hour: number): { heaven: string; earth: string } {
    // 시간별 지지
    const timeBranches = [
      '자', // 23-01시
      '축', // 01-03시
  ...
  static calculateDaeun(
    ...
    const daeunList: Array<{ age: number; pillar: { heaven: string; earth: string } }> = [];
  ...
  // 세운 계산 (현재 년도 기준)
  static calculateSaeun(currentYear: number): { heaven: string; earth: string } {
